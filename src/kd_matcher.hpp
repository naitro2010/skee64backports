...
class kd_matcher {
    public:
        ...
        kd_matcher(Morpher::Vector3* points, int count)
            : root(nullptr), points(nullptr) {
            if (count <= 0)
                return;
        ...
        class kd_node {
            public:
                ...
                std::pair<Morpher::Vector3*, int> add(const std::pair<Morpher::Vector3*, int>& point, int depth) {
                    ...
                    if (domore) {
                        if (more)
                            return more->add(point, depth + 1);
                        else
                            more = new kd_node(point);
                    } else {
                        if (less)
                            return less->add(point, depth + 1);
                        else
                            less = new kd_node(point);
                    }
    ...
    public:
        ...
        class kd_node {
            public:
                ...
                void add(Morpher::Vector3* point, int point_index, int depth) {
                    ...
                    if (domore) {
                        if (more)
                            ...
                            more = new kd_node(point, point_index);
                    } else {
                        if (less)
                            return less->add(point, point_index, depth + 1);
                        else
                            less = new kd_node(point, point_index);
                    }
                }
                ...
                void find_closest(Morpher::Vector3* querypoint, std::vector<kd_query_result>& queryResult, float radius, float& mindist,
                                  int depth = 0) {
